...
name = StringVar()                        # for name
age = StringVar()                         # for age
doj = StringVar()                         # for doj
gender = StringVar()                      # for gender
email = StringVar()                       # for email
contact = StringVar()                     # for contact
...
# 6 Setting title and label
                                                             # for title                                        
title = Label(entries_frame, text="Employee Record Management System", font=("Calibri", 20, "bold"), bg="#535c68", fg="white")
title.grid(row=0, columnspan=4, padx=300, pady=20, sticky="w")
                                                             # for name 
...
txtAddress = Text(entries_frame, width=94, height=5, font=("Calibri", 16))
...
def getData(event):
    ...
    txtAddress.insert(END, row[7])
                                                             # To display information of all employees
def dispalyAll():
...
def update_employee():
    if txtName.get() == "" or txtAge.get() == "" or txtDoj.get() == "" or txtEmail.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
            1.0, END) == "":
        messagebox.showerror("Erorr in Input", "Please Fill All Requried Details")
        return
...
# 8 Defining  buttons
                                                                 ...
                                                                 # For Add
btnAdd = Button(entries_frame, command=add_employee, text="Add Details", width=15, font=("Calibri", 16, "bold"), fg="white",
                bg="#16a085", bd=0).grid(row=1, column=4)
                                                                 # For Edit
btnEdit = Button(entries_frame, command=update_employee, text="Update Details", width=15, font=("Calibri", 16, "bold"),
                 fg="white", bg="#2980b9",bd=0).grid(row=2, column=4, padx=10)
                                                                 # For Delete
btnDelete = Button(entries_frame, command=delete_employee, text="Delete Details", width=15, font=("Calibri", 16, "bold"),
                   fg="white", bg="#c0392b",bd=0).grid(row=3, column=4, padx=10)
                                                                 # For Clear
btnClear = Button(entries_frame, command=clearAll, text="Clear Details", width=15, font=("Calibri", 16, "bold"), fg="white",
                  bg="#f39c12",bd=0).grid(row=4, column=4, padx=10)


# 9 Settig Table Frame
...
# 10 Executing program continuously
